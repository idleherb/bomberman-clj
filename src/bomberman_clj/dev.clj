(ns bomberman-clj.dev
  (:require [bomberman-clj.arena :as arena]
...
(defn key-to-player-action
  [key]
  (case key
    :up {:player-id :player-1, :action #(arena/move % :player-1 :north)}
    :right {:player-id :player-1, :action #(arena/move % :player-1 :east)}
    :down {:player-id :player-1, :action #(arena/move % :player-1 :south)}
    :left {:player-id :player-1, :action #(arena/move % :player-1 :west)}
    \space {:player-id :player-1, :action #(arena/plant-bomb % :player-1 (System/currentTimeMillis))}  ; fix call
    \w {:player-id :player-2, :action #(arena/move % :player-2 :north)}
    \d {:player-id :player-2, :action #(arena/move % :player-2 :east)}
    \s {:player-id :player-2, :action #(arena/move % :player-2 :south)}
    \a {:player-id :player-2, :action #(arena/move % :player-2 :west)}
    :tab {:player-id :player-2, :action #(arena/plant-bomb % :player-2 (System/currentTimeMillis))}  ; fix call
    nil))
...
(defn draw-arena
  ...
  (let [scr (s/get-screen :swing)
        arena (arena/init-arena width height players)]
    (s/in-screen scr
      (loop [arena arena
             key nil]
        (when (not= key :escape)
                ...
                (s/put-string
                  ...
                  (let [player (dev-cell-player cell)]
                    ...
                    (cond
                      ...
                      (not (nil? (cells/cell-bomb-id cell))) "X"
